...
def rhs_conservative(
    ...
    u_r: np.ndarray, 
    u_s: np.ndarray,
    surface_correction: np.ndarray | None = None
) -> np.ndarray:
    """
    Conservative (Divergence) Formulation Volume RHS:
        dq/dt = - (1/J) * [ Dr * (J * u_r * q) + Ds * (J * u_s * q) ] + surface_correction
    """
    flux_r = J * u_r * q
    flux_s = J * u_s * q
    ...
    if surface_correction is not None:
        rhs_val += surface_correction
        
    return rhs_val


def rhs_split2_twoterm(
    q: np.ndarray, 
    Dr: np.ndarray, 
    Ds: np.ndarray, 
    J: np.ndarray, 
    ...
    Split2 (Two-Term Split, Energy Stable) Formulation Volume RHS:
        dq/dt = -0.5 * (1/J) * [ Dr*(J*u_r*q) + J*u_r*(Dr*q) + Ds*(J*u_s*q) + J*u_s*(Ds*q) ] + surface_correction
    """
...
def rhs_split3_threeterm(
    ...
    Split3 (Three-Term Split, Mass Conserving) Formulation Volume RHS:
        dq/dt = -0.5 * (1/J) * [ Dr*(J*u_r*q) + J*u_r*(Dr*q) + q*Dr*(J*u_r) + Ds*(J*u_s*q) + J*u_s*(Ds*q) + q*Ds*(J*u_s) ] + surface_correction
    """
    div_ur = (Dr @ (J * u_r).T).T
    div_us = (Ds @ (J * u_s).T).T
